...
export default class Room {
    ...
    players = new Map();
    roundCounter = 0;
    maxRoundCount = 10; // total amount of questions to be asked
    laneCount = 3;
    hostSocket;
    laneObjects; // generated in refreshLaneObjects call in constructor
    state;
    destroy = () => { };
    questionInDBCount;
    constructor() {
        ...
        this.questionInDBCount = db.prepare("SELECT COUNT(*) as res FROM questions").get().res;
    }
    newPlayer(name, socket) {
        ...
        const assignedColours = Array.from(this.players.values()).map((p) => p.colour);
        const availableColours = tailwindColors.filter((color) => !assignedColours.includes(color));
        const colour = availableColours.length > 0
            ? availableColours[0]
    ...
    refreshLaneObjects() {
        ...
        this.laneObjects = new Array(this.maxRoundCount * 2 + 1).fill(null).map(() => {
            const arr = Array(this.laneCount).fill(null).map(() => this.generateLaneObject());
            if (arr.every(a => a == "obstacle"))
                arr[randomInt(0, 2)] = "nothing";
            return arr;
        });
    }
    generateLaneObject() {
        ...
        if (random < 0.4)
            return "question"; // 40% chance of question
        if (random < 0.6)
            return "obstacle"; // 20% chance of obstacle
        return "nothing"; // 40% chance of nothing
    }
    respondToUserMessage(username, msg, socket) {
        // get player
        ...
        switch (msg.messageType) {
            case "changeLane":
                ...
                console.log(`changing lane to ${msg.data.lane}`);
                player.lane = msg.data.lane;
                ...
                break;
            case "answerQuestion":
                player.selectedAnswer = msg.data.answerIndex;
                break;
            default:
    ...
    respondToHostMessage(msg, io) {
        ...
        switch (msg.messageType) {
            ...
            case "endQuestion":
                ...
                break;
            default:
                console.error(`Cannot recognize host message type: ${JSON.stringify(msg)}\n `);
                this.hostSocket?.emit("message", {
                    messageType: "badMessage",
    ...
    enterUpdatePhase() {
        ...
        this.players.forEach((player, name) => {
            ...
            const upcomingLaneInfo = this.laneObjects[this.roundCounter + delta];
            ...
            console.log("Sending a car colour update message to the user", name, "car colour:", player.colour);
            player.socket.emit("message", {
        ...
        }, 1000 * 10 * 1); // 1 minute delay
    }
    generateQuestion(player, collidedWith) {
        if (collidedWith != "question")
            return undefined;
    ...
    endLaneChoosing() {
        ...
        this.players.forEach((player, name) => {
            const delta = this.getPositionDelta(player);
            const upcomingLaneObj = this.laneObjects[this.roundCounter + delta][player.lane];
            ...
            const toUserQuestion = question != undefined
                ? {
    ...
    endQuestioningPhase() {
        if (this.state == "endgame")
            return; // game already ended
        if (this.state !== "inGame - questioning")
            return; // question has already ended
