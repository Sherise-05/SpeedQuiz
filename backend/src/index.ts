...
app.post("/join", (req, res) => {
  ...
  if (req.body.gameCode && rooms[req.body.gameCode] !== undefined) {
    ...
    res.redirect(
      `${FRONTEND_URL}/lobby?roomID=${req.body.gameCode}&host=false&username=${req.body.username}`,
    );
  } else {
    console.log(
      "Client redirected from /join due to room not being specified or room not existing",
    );
...
app.get("/testing", async (req, res) => {
  ...
  res.send(
    `<p>Amount of questions in the DB: ${(db.prepare("SELECT COUNT(*) as res FROM questions").get() as {res: number}).res}</p>`,
  );
});
...
io.on("connection", (socket) => {
  ...
  if (
    ((queryParams.isHost == undefined || queryParams.isHost == "false") &&
  ...
  else rooms[queryParams.roomID].setHostSocket(socket);

  socket.on("message", (msg) => {
    console.log(`Message: ${JSON.stringify(msg)}`);

    if (queryParams.isHost == undefined || queryParams.isHost == "false")
      rooms[queryParams.roomID as string].respondToUserMessage(
        queryParams.name as string,
        msg,
        socket,
      );
    else rooms[queryParams.roomID as string].respondToHostMessage(msg, io);
  });
});
